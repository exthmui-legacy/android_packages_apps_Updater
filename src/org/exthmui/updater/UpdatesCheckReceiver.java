...
public class UpdatesCheckReceiver extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, Intent intent) {
        ...
        final File json2 = Utils.getCachedNoticeList(context);
        final File jsonNew = new File(json.getAbsolutePath() + UUID.randomUUID());
        final File jsonNew2 = new File(json.getAbsolutePath() + UUID.randomUUID());
        String url = Utils.getServerURL(context,true);
        String url2 = Utils.getServerURL(context,false);
        DownloadClient.DownloadCallback callback = new DownloadClient.DownloadCallback() {
            ...
            public void onSuccess(File destination) {
                try {
                    if (json.exists() && Utils.checkForNewUpdates(json, jsonNew, true)) {
        ...
        try {
            DownloadClient downloadClient = new DownloadClient.Builder()
                    ...
                    .build();
            DownloadClient downloadClient2 = new DownloadClient.Builder()
                    .setUrl(url2)
                    .setDestination(jsonNew2)
